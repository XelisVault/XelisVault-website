...
nobody occupies that point. The first is agent commerce. Coinbase revived
...
cards. Emission and block production stay within reach of anyone who owns
...
\midrule
Mining algorithm & RandomX, 4 MiB light mode & proven by years of attack; processor only, by design \\
Block cadence & 2 seconds & near-immediate inclusion; the DAG absorbs the orphan rate \\
Block ordering & fast-converging DAG (GHOSTDAG family) & security grows with block volume, not only depth \\
Flow anonymity & ring of 16, masked amounts & three times the decoy set of current CryptoNote defaults \\
Transport network & Dandelion++, Tor by default & IP-address analysis is the number one practical flaw of private chains \\
Finality & Ring checkpoints (Section~\ref{sec:ring}) & economic irreversibility in under 6 seconds \\
Target throughput, phase 1 & 100 to 300 transactions per second & parallelizable ring verification; adaptive size \\
Storage & pruned DAG & a full node must fit on a personal computer \\
\bottomrule
\end{tabularx}
\end{table}

Pruning deserves a full sentence, because it conditions the entire social
contract: keep of history only the recent state and the cryptographic
proofs of the past, so that a full node stays within the bounds of a
personal computer. A network whose nodes required a disk bay would exclude
exactly the participants it claims to serve. Dandelion++ stem propagation
and Tor transport close the door on IP-address analysis, which remains the
great practical weakness of badly configured private networks:
...
and a dedicated audit before any main network. The specification is
precise, the literature is public and solid, and none of the primitives
