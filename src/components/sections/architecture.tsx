...
const LAYERS = [
  {
    ...
    desc: 'VLT holders propose and vote on every protocol parameter. 48h timelock enforced on-chain, with a Guardian multisig for emergency pause.',
    contracts: ['OracleGovernance', 'GovernanceVault', 'Governor', 'Timelock', 'GuardianMultisig'],
    color: 'vlt',
    icon: Vote,
  },
  {
    ...
    desc: 'The user-facing contracts: lending, swapping, savings, auctions, RWA, treasury, insurance, payroll, mixer, chat. All encrypted, all on XELIS.',
    contracts: ['VaultEngineV3', 'VaultSwapV2', 'PSM', 'SavingsRate', 'SealedBidAuction', 'AssetVault', 'TreasuryVault', 'PrivacyMixer', 'VaultChat', 'ComplianceModule'],
    color: 'vault',
    icon: Layers,
  },
  {
    ...
    desc: 'A decentralized price oracle secured by VLT staking. Anyone can be a provider, slashing deters manipulation, median aggregation every 5 blocks.',
    contracts: ['StakedOracle', 'InterestRateModel', 'XelisVaultMiner', 'MinerPool'],
    color: 'xusd',
    icon: Cpu,
  },
  {
    ...
    name: 'Token Layer',
    tag: 'The two assets',
    desc: 'VLT — 10M fixed supply, deflationary governance token burned by fees and slashing. xUSD — USD-pegged stablecoin minted by PSM and overcollateralized vaults.',
    contracts: ['VLTToken', 'xUSD'],
    color: 'vlt',
    icon: Coins,
  },
  {
    ...
    name: 'Infrastructure',
    tag: 'XELIS BlockDAG',
    desc: 'The base layer: XELIS native homomorphic encryption (Twisted ElGamal), 5-second block time, contract registry, upgrade pattern, reentrancy guards.',
    contracts: ['ContractRegistry', 'Upgradeable', 'ReentrancyGuard', 'Pausable', 'FaucetContract'],
    color: 'vault',
    icon: Server,
  },
] as const
        ...
        <div className="max-w-3xl">
          ...
          <Reveal delay={0.2}>
            <p className="mt-8 text-lg text-muted-foreground leading-relaxed">
              33 smart contracts, organized in 5 layers. Each layer secures the one above,
