...
export function Contracts() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-none border border-border bg-background/60 pl-9 pr-3.5 py-2.5 text-sm outline-none focus:border-vault/50"
          />
        </div>
        <div className="flex gap-1.5 overflow-x-auto hide-scrollbar pb-1">
          {categories.map((c) => (
            <button
              ...
              className={`shrink-0 rounded-none px-3 py-1.5 text-[11px] font-mono uppercase tracking-wider transition-all${
      ...
      <p className="text-[11px] text-muted-foreground/70 leading-relaxed border-l-2 border-vault/40 pl-3">
...
function ContractCard({ meta, hash, index }: { meta: ContractMeta; hash: string | null; index: number }) {
  return (
    <motion.div
      ...
      className={`rounded-none border p-4${meta.deployed ? 'border-border bg-card/40' : 'border border-border/60 bg-transparent'}`}
    >
