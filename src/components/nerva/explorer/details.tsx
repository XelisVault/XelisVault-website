...
function Row({ label, children, mono = true }: { label: string; children: React.ReactNode; mono?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2 border-b border-white/6 last:border-0">
      <span className="font-mono text-[9.5px] uppercase tracking-[0.16em] text-[oklch(0.55_0.025_250)] pt-0.5 shrink-0">{label}</span>
...
function Badge({ children, tone = 'cyan' }: { children: React.ReactNode; tone?: 'cyan' | 'violet' | 'green' | 'neutral' | 'red' }) {
  const tones = {
    cyan: 'bg-[oklch(0.82_0.115_215)]/14 text-[oklch(0.85_0.1_220)] border-[oklch(0.82_0.115_215)]/25',
    violet: 'bg-[oklch(0.72_0.15_290)]/16 text-[oklch(0.8_0.13_290)] border-[oklch(0.72_0.15_290)]/28',
    green: 'bg-[oklch(0.75_0.14_160)]/14 text-[oklch(0.8_0.11_160)] border-[oklch(0.75_0.14_160)]/25',
...
function Loading({ label }: { label: string }) {
  return (
    <div className="panel-nerva rounded-xl p-8 flex flex-col items-center gap-3">
      <Loader2 className="w-5 h-5 text-[oklch(0.82_0.115_215)] animate-spin" />
      <span className="font-mono text-[11px] text-[oklch(0.6_0.025_250)]">{label}…</span>
    </div>
  )
}
    ...
    <div className="panel-nerva rounded-xl overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-white/8">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="ml-1 text-white/30 hover:text-[oklch(0.82_0.115_215)] transition-colors"
      ...
      <div className="px-4 py-3 max-h-[calc(100vh-320px)] overflow-y-auto">
        ...
        {h.miner_tx_hash && (
          <Row label="Miner tx">
            <button
              ...
              className="text-[oklch(0.82_0.115_215)] hover:text-[oklch(0.9_0.1_215)] transition-colors underline decoration-dotted underline-offset-2"
            >
        ...
        {/* transactions */}
        {loadingTxs && (
          <div className="mt-3 flex items-center gap-2 font-mono text-[10px] text-[oklch(0.6_0.025_250)]">
            <Loader2 className="w-3.5 h-3.5 animate-spin" /> loading transactions…
          </div>
        )}
        {transfers.length > 0 && (
          <div className="mt-4">
            <div className="font-mono text-[9.5px] uppercase tracking-[0.16em] text-[oklch(0.55_0.025_250)] mb-2 flex items-center gap-1.5">
            ...
            <div className="space-y-2">
              {transfers.map((t) => (
                <button
                  ...
                  className="w-full rounded-lg border border-white/8 bg-white/[0.03] hover:border-[oklch(0.82_0.115_215)]/40 hover:bg-[oklch(0.82_0.115_215)]/6 px-3.5 py-2.5 text-left transition-all group"
                >
                  ...
                  <div className="mt-1 font-mono text-[9.5px] text-[oklch(0.55_0.02_255)]">
        ...
        {!loadingTxs && transfers.length === 0 && (h.num_txes ?? 0) === 0 && (
          <div className="mt-4 rounded-lg border border-white/8 bg-white/[0.02] px-3.5 py-3 font-mono text-[10px] text-[oklch(0.55_0.025_250)] flex items-center gap-2">
...
export function TxDetail({ hash, onClose, onBackToBlock }: { hash: string; onClose: () => void; onBackToBlock: (hash: string) => void }) {
  ...
  return (
    <div className="panel-nerva rounded-xl overflow-hidden">
      <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-white/8">
        <HashIcon className="w-4 h-4 text-[oklch(0.82_0.115_215)]" />
        <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-[oklch(0.6_0.025_250)]">Transaction</span>
      ...
      <div className="px-4 py-3 max-h-[calc(100vh-320px)] overflow-y-auto">
        ...
        {tx.in_pool ? (
          <Row label="Status"><span className="text-[oklch(0.8_0.11_160)]">waiting in the mempool — not yet in a block</span></Row>
        ) : (
          <>
            <Row label="Block">
              {block ? (
                <span className="inline-flex items-center gap-2">
                  <button onClick={() => onBackToBlock(block.block_header.hash)} className="inline-flex items-center gap-1.5 text-[oklch(0.82_0.115_215)] hover:text-[oklch(0.9_0.1_215)] transition-colors">
            ...
            <Row label="Timestamp">{tx.block_timestamp ? formatTimestamp(tx.block_timestamp) : '—'}</Row>
          </>
        )}
        ...
        {isCoinbase ? (
          <>
            ...
            <Row label="Amount">
              <span className="text-[oklch(0.82_0.115_215)] font-semibold">{formatXnv(coinbaseAmount)} XNV</span>
            </Row>
            <Row label="Unlock">
          ...
          <>
            ...
            <Row label="Amount">
              ...
              <span className="block text-[10px] text-[oklch(0.55_0.02_255)] mt-0.5">RingCT hides transfer amounts — only the network can verify no inflation</span>
            </Row>
            <Row label="RingCT type">{rctType != null ? `type ${rctType}` : '—'}</Row>
            {tx.json?.unlock_time ? (
              <Row label="Unlock">
        ...
        <div className="mt-4 rounded-lg border border-white/8 bg-white/[0.02] px-3.5 py-3">
          <div className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.16em] text-[oklch(0.55_0.025_250)]">
          ...
          <div className="mt-2.5 space-y-1.5 font-mono text-[11px]">
            {parsed.txPubkey && (
              <div className="flex items-center justify-between gap-3">
                <span className="text-[oklch(0.6_0.025_250)] text-[9.5px] uppercase tracking-[0.12em]">tx pubkey</span>
            ...
            {parsed.paymentIdLong && (
              <div className="flex items-center justify-between gap-3">
                <span className="text-[oklch(0.8_0.13_290)] text-[9.5px] uppercase tracking-[0.12em]">payment id (clear)</span>
                <span className="text-[oklch(0.8_0.13_290)]">{parsed.paymentIdLong.slice(0, 16)}… <CopyButton text={parsed.paymentIdLong} /></span>
              </div>
            )}
            {parsed.paymentIdShort && (
              <div className="flex items-center justify-between gap-3">
                <span className="text-[oklch(0.55_0.025_250)] text-[9.5px] uppercase tracking-[0.12em]">payment id (encrypted)</span>
        ...
        {outputs.length > 0 && outputs[0]?.target?.key && (
          <div className="mt-3">
            <div className="font-mono text-[9.5px] uppercase tracking-[0.16em] text-[oklch(0.55_0.025_250)] mb-2">
            ...
            <div className="space-y-1">
              {outputs.slice(0, 4).map((o, i) => (
                <div key={i} className="font-mono text-[10px] text-white/55 flex items-center gap-2">
                  <span className="text-[oklch(0.5_0.02_255)]">#{i}</span>
                  {o.target?.key ? shortenHash(o.target.key, 10, 6) : '—'}
                  {isCoinbase && i === 0 && <span className="text-[oklch(0.8_0.11_160)]">{formatXnv(o.amount)} XNV</span>}
                </div>
              ))}
              {outputs.length > 4 && <div className="font-mono text-[9.5px] text-[oklch(0.5_0.02_255)]">+{outputs.length - 4} more…</div>}
            </div>
          </div>
        )}
        ...
        <div className="mt-4">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 font-mono text-[10.5px] text-white/45 hover:text-[oklch(0.82_0.115_215)] transition-colors uppercase tracking-[0.1em]"
          >
